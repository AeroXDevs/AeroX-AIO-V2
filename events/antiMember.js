...
async function handleGuildBanAdd(member) {
  try {
    ...
    await member.guild.members.ban(executorMember.id, { reason: 'Anti Ban | Not Whitelisted' });
...
async function handleGuildBanRemove(member) {
  try {
    ...
    await member.guild.members.ban(executorMember.id, { reason: 'Anti Unban | Not Whitelisted' });
...
async function handleGuildMemberAdd(member) {
  try {
    ...
    await member.guild.members.ban(executorMember.id, { reason: 'Anti Bot | Not Whitelisted' });

    if (autorecovery === true) {
      await member.guild.members.kick(target.id, {
...
async function handleGuildMemberRemove(member) {
  try {
    ...
    await member.guild.members.ban(executorMember.id, { reason: 'Anti Kick | Not Whitelisted' });
...
async function handleGuildMemberPrune(member) {
  ...
  try {
    ...
    await member.guild.members.ban(executorMember.id, { reason: 'Anti Prune | Not Whitelisted' });
...
async function handleGuildMemberUpdate(oldMember, newMember) {
  try {
    ...
    if (!executorMember || (!executorMember.permissions.has('ManageRoles') && !executorMember.permissions.has('Administrator'))) {
      return;
    }
    ...
    await newMember.guild.members.ban(executorMember.id, { reason: 'Anti Member Update | Not Whitelisted' });
