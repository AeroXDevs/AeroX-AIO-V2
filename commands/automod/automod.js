        ...
        const buildGuideContainer = () => {
            ...
            container.addTextDisplayComponents(
                ...
                new TextDisplayBuilder().setContent(
                    `${emoji.util.arrow} \`${prefix}automod anti pornography enable/disable\`\n` +
                    'Enable/Disable anti-pornography filter.\n\n' +
                    `${emoji.util.arrow} \`${prefix}automod anti message spam enable/disable\`\n` +
                    'Enable/Disable message spam filter.\n\n' +
                    `${emoji.util.arrow} \`${prefix}automod anti mention spam enable/disable\`\n` +
                    'Enable/Disable mention spam filter.\n\n' +
                    `${emoji.util.arrow} \`${prefix}automod anti toxicity enable/disable\`\n` +
                    'Enable/Disable toxicity filter.\n\n' +
                    `${emoji.util.arrow} \`${prefix}automod config\`\n` +
                    'Configure automod settings.\n\n' +
                    `${emoji.util.arrow} \`${prefix}automod reset\`\n` +
                    ...
                    case 'pornography':
                        switch (args[2]) {
                            ...
                            case 'disable':
                                ...
                                break;
                            default:
                                message.channel.send({ components: [buildGuideContainer()], flags: MessageFlags.IsComponentsV2 });
                                break;
                        }
                        break;
                    case 'message':
...
async function automodConfig(client, message) {
    ...
    if (pornRule && pornRule.creatorId === client.user.id && pornRule.enabled === true) {
        automodArray.push(`${emoji.util.enabled} Anti Pornography`)
    } else {
        automodArray.push(`${emoji.util.disabled} Anti Pornography`)
    }

    if (toxicRule && toxicRule.creatorId === client.user.id && toxicRule.enabled === true) {
        automodArray.push(`${emoji.util.enabled} Anti Toxicity`)
    } else {
        automodArray.push(`${emoji.util.disabled} Anti Toxicity`)
    }

    if (messageRule && messageRule.creatorId === client.user.id && messageRule.enabled === true) {
        automodArray.push(`${emoji.util.enabled} Anti Message Spam`)
    } else {
        automodArray.push(`${emoji.util.disabled} Anti Message Spam`)
    }

    if (mentionRule && mentionRule.creatorId === client.user.id && mentionRule.enabled === true) {
        automodArray.push(`${emoji.util.enabled} Anti Mention Spam`)
    } else {
        automodArray.push(`${emoji.util.disabled} Anti Mention Spam`)
    }
    ...
    container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
            `# ${emoji.id.automod} Automod Configuration\n*Current server automod settings*`
        )
    );
    ...
    container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent('## Active Modules')
    );

    container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent(automodArray.join('\n'))
    );
    ...
    container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
            `*Thanks For Selecting ${client.user.username}*`
        )
    );
