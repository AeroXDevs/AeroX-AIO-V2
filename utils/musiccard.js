...
class MusicCard {
                ...
                try {
                        ...
                        for (const fontPath of fontPaths) {
                                try {
                                        ...
                                        console.log(
                                                `[MusicCard] Fonts registered successfully from: ${fontPath.path} (${fontPath.context})`,
                                        );
                        ...
                        if (!fontsRegistered) {
                                console.warn(
                                        '[MusicCard] Could not register custom fonts from any path. Using system defaults.',
                                );
                        }
                } catch (e) {
        ...
        async drawArtwork(ctx, track, x, y, size) {
                ...
                try {
                        const artworkUrl =
                                track?.thumbnail || track?.info?.artworkUrl || track?.pluginInfo?.artworkUrl;
                        if (artworkUrl) {
        ...
        async createMusicCard(track, position = 0, options = {}) {
                ...
                const textBlockHeight = artworkSize;
                let currentY = artworkY + 15;

                
                const source = track?.sourceName || track?.info?.sourceName || 'Unknown';
                ctx.save();
                ...
                const title = track?.title || track?.info?.title || 'Unknown Title';
                ...
                const artist = track?.author || track?.info?.author || 'Unknown Artist';
                ...
                const duration = track?.length || track?.duration || track?.info?.duration || 0;
