...
class CV2Command extends Command {
  ...
  buildMainContainer(username) {
    ...
    container.addActionRowComponents(
      new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder().setCustomId('main_select_menu').setPlaceholder('Choose a demo option...').addOptions(
          new StringSelectMenuOptionBuilder().setLabel('Toggle Spoiler Mode').setValue('spoiler').setDescription('Hide/show content with spoilers'),
          new StringSelectMenuOptionBuilder().setLabel('Regenerate Content').setValue('regenerate').setDescription('Create new random content')
        )
      )
    );
    container.addActionRowComponents(
